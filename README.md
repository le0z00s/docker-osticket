...
Docker image for running upstream version of [OSTicket](http://osticket.com/).

This image has been created from the docker-osticket image by Martin Campbell.
The original image as been created by [Petter A. Helset](mailto:petter@helset.eu).
...
As in [Martin Campbell's](https://github.com/CampbellSoftwareSolutions/docker-osticket) image, `setup/` directory has been renamed as `setup_hidden/` and the file system permissions deny nginx access to this
...
This image currently supports three volumes. None of these need to be used if you do not require them.
